...
from patterns import pattern_check, expected, alternative
...
def trajectory(id, setup):
    ...
    # Dictionary for average pattern counts over k trajectories
    pattern_avg = {}
    # Repeats generation k times
    for r in range(setup['k']):
        ...
        # Copies the first pattern's dictionary
        if r == 0:
            pattern_avg = patterns.copy()
            # Divides by k to get the correct contribution to average
            for key in patterns.keys():
                pattern_avg[key] = 1/k_repeats * pattern_avg[key]
        # Adds contribution of this trajectory to pattern_avg
        else:
            for key in patterns.keys():
                pattern_avg[key] += 1/k_repeats * patterns[key]
    ...
    if save:
        ...
        with open(displaced_null_markov.joinpath('varying').joinpath('repeated_traj.csv'), 'a', newline='') as file:
            data = [n, k_repeats, lmbd, phi, eps, gamma, theta_rough, offset, mpn, np.sum(x)/len(x), wsp] + [val for val in pattern_avg.values()]
            z = csv.writer(file)
    ...
    if save2:
        ...
        with open(displaced_null_markov.joinpath('varying').joinpath(
                'repeated_exp.csv'), 'a', newline='') as file:
    ...
    if save3:
        # Opens the file in append mode
        with open(displaced_null_markov.joinpath('varying').joinpath(
                'repeated_exp_mus.csv'), 'a', newline='') as file:
            data = [n, k_repeats, lmbd, phi, eps, gamma, theta_rough, offset, mpn, mpn_pat, FI_pat] + [val for val in
                                                                                            alt_mus.values()]
            z = csv.writer(file)
            # Appends data onto the file
            z.writerow(data)
    ...
    return pattern_avg, alt_mus, alt_expected
...
setup = {
    'N': 500,  # No. of samples to generate
    'n': 1000000,  # No. of ancillary systems
    'theta': 0.2,  # True value of theta for trajectory generation
    'lambda': 0.8,  # Transition parameter, 0<=lmbd<=1
    'phi': pi / 4,  # Phase parameter
    'initial': True,  # Controls initial estimation; eps and gamma are irrelevant if True
    'eps': 0.067,  # Prop. of traj. to use in initial est.
    'gamma': 2.5,   # Controls how far the absorber is offset, gamma>1
    'k': 10     # No. of times the final estimation is repeated
}
###########
## Setup ##
###########
...
if __name__ == '__main__':
    ...
    # Samples used in initial estimation
    n_init = int(np.floor(n**(1-eps)))
    # Samples used in final estimation
    n_final = n - n_init
    # Final estimation repeated k times
    n_total = n_final*k + n_init

    # Process ids
    ids = np.arange(setup['N'])

    # Useful information
    print(f'Running with N={N}, n={n}, k={k}')
    if setup['initial']:
